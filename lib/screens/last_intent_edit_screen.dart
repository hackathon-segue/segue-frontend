...
/// [StructuredIntentVocabulary]'s fixed 17-key/value list rather than free
/// text, so a save can never produce a key/value pair the `/decide` API
/// contract wouldn't accept.
class LastIntentEditScreen extends StatefulWidget {
  const LastIntentEditScreen({required this.customer, required this.cartItem, super.key});
...
class _LastIntentEditScreenState extends State<LastIntentEditScreen> {
  ...
  void didChangeDependencies() {
    ...
    final StructuredIntent intent = session.state.structuredIntent ?? StructuredIntent.empty();
  ...
  bool get _hasIncompleteBucket => _buckets.entries.any(
    (MapEntry<String, _ConditionBucket> e) => e.value != _ConditionBucket.none && _values[e.key] == null,
  );
  ...
  Future<void> _save(LastIntentSessionController session) async {
    ...
    final StructuredIntent current = session.state.structuredIntent ?? StructuredIntent.empty();
  ...
  Widget build(BuildContext context) {
    ...
    return StaffAppShell(
      ...
      body: _saving ? const AppStateView.loading(title: '로딩중...') : _buildForm(session),
    );
  }
    ...
    return Column(
      ...
      spacing: 16,
        children: <Widget>[
          ...
          const Text('AI가 정리한 조건을 고객과 함께 확인하고 필요한 항목을 수정하세요.', style: StaffText.body12),
          SectionCard(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              spacing: 8,
              ...
              children: <Widget>[
                ...
                Wrap(
                  ...
                  children: <Widget>[
                    for (final String key in StructuredIntentVocabulary.attributeLabels.keys)
                      FilterChip(
                        label: Text(StructuredIntentVocabulary.attributeLabel(key)),
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                _TriStateRow(
                  ...
                  onChanged: (bool? value) => setState(() => _canVisitOtherStore = value),
                ),
              ],
            ),
          ),
          Wrap(
            ...
            children: <Widget>[
              ...
              StaffButton(
                label: '저장',
                variant: _hasIncompleteBucket ? StaffButtonVariant.secondary : StaffButtonVariant.primary,
    ...
    return Padding(
      ...
      child: Wrap(
        ...
        children: <Widget>[
          SizedBox(
            width: 120,
            child: Text(StructuredIntentVocabulary.attributeLabel(attributeKey), style: StaffText.body12),
          ),
          SizedBox(
            width: 160,
            child: DropdownButtonFormField<_ConditionBucket>(
              ...
              items: const <DropdownMenuItem<_ConditionBucket>>[
                DropdownMenuItem<_ConditionBucket>(value: _ConditionBucket.none, child: Text('미지정')),
                DropdownMenuItem<_ConditionBucket>(value: _ConditionBucket.essential, child: Text('필수')),
                DropdownMenuItem<_ConditionBucket>(value: _ConditionBucket.preferred, child: Text('선호')),
                DropdownMenuItem<_ConditionBucket>(value: _ConditionBucket.negotiable, child: Text('양보 가능')),
              ],
            ...
            SizedBox(
              ...
              child: DropdownButtonFormField<String>(
                ...
                items: <DropdownMenuItem<String>>[
                  for (final String option in StructuredIntentVocabulary.attributeValues[attributeKey]!)
                    DropdownMenuItem<String>(
                      value: option,
                      child: Text(StructuredIntentVocabulary.attributeValueLabel(attributeKey, option)),
                    ),
                ],
