...
class MockSegueRepository implements SegueRepository {
  ...
  Future<StructureIntentResponse> structureIntent(
    ...
    } else if (request.utterance.contains('다이아몬드') &&
        request.utterance.contains('직사각')) {
      // SCHEMA.md 페르소나 1 (디자인 유지형) → decide()가 COMPARISON_EXPERIENCE/SKU2로 매핑.
      essentialConditions = <String, String>{'silhouette': '사각'};
    } else if (request.utterance.contains('가죽') ||
        request.utterance.contains('비세토스')) {
    ...
    } else if (request.utterance.contains('색이나 소재') ||
        request.utterance.contains('그대로인')) {
  ...
  Future<StructureIntentResponse> submitFollowUpAnswer(
    ...
    final bool stillUnclear =
        request.followUpAnswer.contains('모르겠') ||
        request.followUpAnswer.contains('글쎄');
  ...
  Future<DecisionResult> decide(DecisionRequest request) async {
    ...
    if (ec['material'] == '가죽') {
      ...
      return const DecisionResult(
        ...
        reason:
            '형태보다 시그니처 소재와 컬러를 더 중요하게 보셨기 때문에, 현재 매장에서 바로 비교 체험할 수 있는 이 제품을 제안합니다.',
    ...
    if (ec['colorFamily'] == '브라운') {
      ...
      return const DecisionResult(
        ...
        difference:
            '오늘 바로 구매하지 않으셔도 괜찮다고 하셔서, 동일 제품을 타 매장에서 확보하는 경로를 우선 제안드립니다.',
        recommendedProduct: null,
