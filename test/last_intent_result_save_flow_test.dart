...
class _SpyRepository extends MockSegueRepository {
  ...
  Future<void> recordConsultationResult({
    ...
    return super.recordConsultationResult(customerId: customerId, result: result);
  }
}
...
void main() {
  ...
  Future<LastIntentSessionController> readySession(int skuId, String productName) async {
  ...
  test('execute() success chains into a local save that succeeds', () async {
    ...
    final List<ConsultationResult> stored = await repository.fetchConsultationResults(customer.id);
    expect(stored, hasLength(1));
    expect(stored.first.productName, 'MCM 백팩');
  });

  test('a failed save surfaces an error, keeps execute()/Card data, and retry saves without duplicating', () async {
    final LastIntentSessionController session = await readySession(1, 'MCM 백팩');

    repository.shouldThrowOnRecord = true;
    await session.execute();
    expect(session.state.executionResponse, isNotNull); // execute() itself succeeded
  ...
  test('retrySaveConsultationResult never re-calls execute()/decide()', () async {
    final LastIntentSessionController session = await readySession(1, 'MCM 백팩');
    repository.shouldThrowOnRecord = true;
    await session.execute();
    final int? resultIdAfterFirstAttempt = session.state.executionResponse?.consultationResultId;
    ...
    expect(session.state.executionResponse?.consultationResultId, resultIdAfterFirstAttempt);
  });

  test('one customer can have multiple independent saved results (product A and B)', () async {
    final LastIntentSessionController sessionA = await readySession(1, 'MCM 백팩');
    final LastIntentSessionController sessionB = await readySession(2, 'MCM 숄더백');
    ...
    expect(stored.map((ConsultationResult r) => r.productName), containsAll(<String>['MCM 백팩', 'MCM 숄더백']));
  });

  test('results recorded for one customerId never leak into another customerId\'s fetch', () async {
    final LastIntentSessionController sessionA = await readySession(1, '고객1 제품');
    await sessionA.execute();
    ...
    final List<ConsultationResult> customer1Results = await repository.fetchConsultationResults(
      customer.id,
    );
    expect(customer1Results, hasLength(1));
    expect(customer1Results.single.productName, '고객1 제품');
    expect(
      customer1Results.any((ConsultationResult r) => r.productName == '고객42 제품'),
      isFalse,
    );
  });

  test('newest consultation is listed first', () async {
    final LastIntentSessionController sessionA = await readySession(1, '먼저 상담한 제품');
    await sessionA.execute();
    await Future<void>.delayed(const Duration(milliseconds: 5));
    final LastIntentSessionController sessionB = await readySession(2, '나중에 상담한 제품');
    await sessionB.execute();
  ...
  test('the saved recommendedPath matches what the Card itself would show (recommendedProduct present)', () async {
    final LastIntentSessionController session = manager.sessionFor(
      customer: customer,
    ...
    expect(stored.single.recommendedPath, session.state.decisionResult!.pathDescription);
  });
}
