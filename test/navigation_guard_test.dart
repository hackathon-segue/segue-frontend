...
void main() {
  ...
  const String popupTitle = '상담을 종료하시겠습니까? 화면에서 이탈할 시\n진행 중인 상담은 종료되지 않습니다.';

  testWidgets('상담이 진행 중이지 않을 때는 사이드바를 눌러도 팝업 없이 바로 이동한다', (
    WidgetTester tester,
  ) async {
    tester.view.physicalSize = const Size(1440, 900);
    tester.view.devicePixelRatio = 1;
  ...
  testWidgets('상담 진행 중 CURRENT SESSION을 누르면 팝업이 뜨고, 바깥을 눌러도 닫히지 않는다', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('"상담으로 돌아가기"는 팝업만 닫고 진행 상태를 그대로 유지한다', (
    WidgetTester tester,
  ) async {
    await reachFollowUpScreen(tester);
  ...
  testWidgets('"상담 종료하고 나가기"는 세션을 종료하고 원래 클릭했던 목적지로 이동한다 '
      '(Home으로 고정 이동하지 않는다)', (WidgetTester tester) async {
    await reachFollowUpScreen(tester);
